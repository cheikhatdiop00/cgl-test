...
class OptesisBilan(models.Model):
    _name = 'optesis.bilan'

    @api.model
    def _get_report_values(self, company, debut, fin):
        
        debut_n = debut - relativedelta(years=+1)
        fin_n = fin - relativedelta(years=+1)
        
         # Calcul AE

        self.env.cr.execute("select abs(sum(query.solde))  from (select sum(sub.balance) solde from \
                            (select * from account_move_line aml, account_account aa, account_move am where aml.account_id = aa.id and aml.move_id = am.id and aml.date <= '"+str(fin)+"' and aml.company_id = '"+str(company)+"') as sub\
        ...
        self.env.cr.execute("select abs(sum(query.solde))  from (select sum(sub.balance) solde from \
                            (select * from account_move_line aml, account_account aa, account_move am where aml.account_id = aa.id and aml.move_id = am.id and aml.date < '"+str(debut)+"' and aml.company_id = '"+str(company)+"') as sub\
                            where sub.code like '2181%' or sub.code like '211%' or sub.code like '2191%' \
                            group by sub.code) as query")
        ...
        self.env.cr.execute("select sum(query.solde) from (select sum(sub.balance) solde from \
                            (select * from account_move_line aml, account_account aa, account_move am where aml.account_id = aa.id and aml.move_id = am.id and aml.date between '"+str(debut)+"' and  '"+str(fin)+"' and aml.company_id = '"+str(company)+"' and aml.company_id = '"+str(company)+"') as sub\
        ...
        self.env.cr.execute("select sum(query.solde) from (select sum(sub.balance) solde from \
                            (select * from account_move_line aml, account_account aa, account_move am where aml.account_id = aa.id and aml.move_id = am.id and aml.date between '"+str(debut_n)+"' and  '"+str(fin_n)+"' and aml.company_id = '"+str(company)+"') as sub\
                            where sub.code like '6%' or sub.code like '7%' or sub.code like '8%' \
                            group by sub.code) as query")

...
class OptesisCompteResultat(models.Model):
    _name = 'optesis.compte.resultat'
    ...
    def _get_report_values(self, company, debut, fin):
        ...
        self.env.cr.execute("select abs(sum(sub.balance)) from \
                            (Select * from account_move_line aml, account_account aa, account_move am  where aml.account_id = aa.id and aml.move_id = am.id and aml.date between '"+str(debut)+"' and  '"+str(fin)+"' and aml.company_id = '"+str(company)+"') as sub\

...
class OptesisTft(models.AbstractModel):
    _name = 'optesis.tft'

    @api.model
    def _get_report_values(self, company, debut, date_fin):
        
        debut_n = debut - relativedelta(years=+1)
        fin_n = date_fin - relativedelta(years=+1)
        
        debut_n2 = debut - relativedelta(years=+2)
        fin_n2 = date_fin - relativedelta(years=+2)
        
        compte = self.env['optesis.compte.resultat']._get_report_values(company, debut, date_fin)
        bilan = self.env['optesis.bilan']._get_report_values(company, debut, date_fin)
        ...
        self.env.cr.execute("select abs(sum(query.solde))  from (select sum(sub.balance) solde from \
                            (select * from account_move_line aml, account_account aa, account_move am where aml.account_id = aa.id and aml.move_id = am.id and aml.date <= '"+str(fin_n)+"' and aml.company_id = '"+str(company)+"') as sub\
        ...
        self.env.cr.execute("select abs(sum(query.solde))  from (select sum(sub.balance) solde from \
                            (select * from account_move_line aml, account_account aa, account_move am where aml.account_id = aa.id and aml.move_id = am.id and aml.date <= '"+str(date_fin)+"' and aml.company_id = '"+str(company)+"') as sub\
        ...
        self.env.cr.execute("select abs(sum(query.solde))  from (select sum(sub.balance) solde from \
                            (select * from account_move_line aml, account_account aa, account_move am where aml.account_id = aa.id and aml.move_id = am.id and aml.date <='"+str(fin_n2)+"' and aml.company_id = '"+str(company)+"') as sub\
